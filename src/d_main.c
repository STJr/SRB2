			...
			if (gamestate == GS_LEVEL && netgame)
			{
				if (maptol & TOL_TD)
				{
					if (splitscreen && camera2.chase)
						P_MoveTDChaseCamera(&players[secondarydisplayplayer], &camera2, false);
					if (camera.chase)
						P_MoveTDChaseCamera(&players[displayplayer], &camera, false);
				}
				else
				{
                    if (splitscreen && camera2.chase)
...
static void IdentifyVersion(void)
{
...
#if !defined (HAVE_SDL) || defined (HAVE_MIXER)
	{
		...
		ms = W_VerifyNMUSlumps(musicpath); // Don't forget the music!
		if (ms == 1)
			D_AddFile(musicpath);
		else if (ms == 0)
			I_Error("File %s has been modified with non-music lumps",musicfile);
