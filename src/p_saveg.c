...
static void ArchiveSectors(save_t *save_p)
{
	...
	for (i = 0; i < numsectors; i++, ss++, spawnss++)
	{
		...
		if (diff)
		{
			...
			if (diff5 & SD_ACTION)
				P_WriteUINT16(save_p, ss->action);
			if (diff5 & SD_ARGS)
			{
				for (j = 0; j < NUM_SCRIPT_ARGS; j++)
					P_WriteINT32(save_p, ss->args[j]);
			}
			if (diff5 & SD_STRINGARGS)
			{
				for (j = 0; j < NUM_SCRIPT_STRINGARGS; j++)
				{
					...
					for (k = 0; k < len; k++)
						P_WriteChar(save_p, ss->stringargs[j][k]);
				}
			}
			if (diff5 & SD_ACTIVATION)
				P_WriteUINT32(save_p, ss->activation);
			if (diff & SD_FFLOORS)
...
static void UnArchiveSectors(save_t *save_p)
{
	...
	for (;;)
	{
		...
		if (diff5 & SD_ACTION)
			sectors[i].action = P_ReadINT16(save_p);
		if (diff5 & SD_ARGS)
		{
			for (j = 0; j < NUM_SCRIPT_ARGS; j++)
				sectors[i].args[j] = P_ReadINT32(save_p);
		}
		if (diff5 & SD_STRINGARGS)
		{
			for (j = 0; j < NUM_SCRIPT_STRINGARGS; j++)
			{
				...
				for (k = 0; k < len; k++)
					sectors[i].stringargs[j][k] = P_ReadChar(save_p);
				sectors[i].stringargs[j][len] = '\0';
			}
		}
		if (diff5 & SD_ACTIVATION)
			sectors[i].activation = P_ReadUINT32(save_p);
	...
	for (i = 0; i < numlines; i++, spawnli++, li++)
	{
		...
		if (diff)
		{
			...
			if (diff2 & LD_ACTIVATION)
				P_WriteUINT32(save_p, li->activation);
		}
	}
	P_WriteUINT32(save_p, 0xffffffff);
}
...
static void UnArchiveLines(save_t *save_p)
{
	...
	for (;;)
	{
		...
		if (diff2 & LD_ACTIVATION)
			li->activation = P_ReadUINT32(save_p);
	}
}
