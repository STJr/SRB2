...
#define SLOPEPARAMS(slope, end1, end2, normalheight) \
	...
	if (gl_backsector)
	{
		INT32 gl_toptexture = NO_TEXTURE_NUM, gl_bottomtexture = NO_TEXTURE_NUM;
		fixed_t texturevpeg;
		...
		// check TOP TEXTURE
		if ((worldhighslope < worldtopslope || worldhigh < worldtop) && gl_toptexture != NO_TEXTURE_NUM)
		{
		...
		if ((worldlowslope > worldbottomslope || worldlow > worldbottom) && gl_bottomtexture != NO_TEXTURE_NUM)
		{
		...
		if (gl_midtexture != NO_TEXTURE_NUM && HWR_BlendMidtextureSurface(&Surf))
		{
			sector_t *front, *back;
	...
	else
	{
		...
		if (gl_midtexture != NO_TEXTURE_NUM && gl_linedef->special != SPECIAL_HORIZON_LINE) // (Ignore horizon line for OGL)
		{
			grTex = HWR_GetTexture(gl_midtexture);
			xscale = FixedToFloat(gl_sidedef->scalex_mid);
			yscale = FixedToFloat(gl_sidedef->scaley_mid);
...
#undef SLOPEPARAMS
	...
	if (!bothceilingssky && !bothfloorssky)
	{
		...
		if (backc1 <= backf1 && backc2 <= backf2)
		{
			...
			if (((backc1 >= frontc1 && backc2 >= frontc2) || (seg->sidedef->toptexture != NO_TEXTURE_NUM))
			&& ((backf1 <= frontf1 && backf2 <= frontf2) || (seg->sidedef->bottomtexture != NO_TEXTURE_NUM)))
			{
				checkforemptylines = false;
				return true;
			}
